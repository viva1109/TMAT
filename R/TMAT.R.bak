source("R/Funcs_tree_related.R",encoding = "UTF-8")
source("R/Funcs_TMAT_statistics.R",encoding = "UTF-8")
source("R/Plot_TMAT.R",encoding = "UTF-8")

TMAT<-function(Data_Read,Pheno,Tree,Taxonomy_Input,Response_Type="categorical",ncore=1,db="silva",TestType="Chi-squared",fdr_cutoff=0.05,plot=T){
  # browser()
  ...
  data_withpheno<-function(pheno,data){
    ...
    data$totalcounts<-apply(data$dataset,1,sum)
    return(data)
  }
  ...
  make_final_data<-function(data){
    # browser()
    propor<-data$dataset/apply(data$dataset,1,sum)
  ...
  TMAT_pval_bygenus_bf<-mclapply(1:length(data_TMAT),function(ind_target){
    ...
    TMAT_func(target,indi_tAnal,type=type,total.reads=Data_Final$totalcounts,conti=conti,cov=Data_Final$covs)
  },mc.cores = ncore)
  ...
  if(TestType=="Chi-squared"){
    result_output<-cbind(data.frame(TMAT_results[,1],stringsAsFactors=F),p.adjust(TMAT_results[,2],method="fdr"))
  }else if(TestType=="F-test"){
    result_output<-cbind(data.frame(TMAT_results[,1],stringsAsFactors=F),p.adjust(TMAT_results[,3],method="fdr"))
  }
  names(result_output)<-c("Genus","FDR")
